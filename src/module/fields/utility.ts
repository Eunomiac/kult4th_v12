...
export function ValueMaxField(initialValue = 0, min = 0, max = 0, hasName?: string) {
  const theseFields = {
  ...
  return hasName
    ? new fields.SchemaField({name: new fields.StringField({required: true, initial: hasName}), ...theseFields})
    : new fields.SchemaField(theseFields);
}
